...
export default function HeroSection() {
  ...
  return <section id="top" ref={hero} aria-label="Anjo and Jasmin wedding photo slideshow" aria-roledescription="carousel" data-playing={playing} className="relative isolate flex min-h-[760px] min-h-svh items-center justify-center overflow-hidden bg-[#263d35] px-6 pb-40 pt-28 text-center text-[#fff4f8]">
    <div className="absolute inset-0 -z-20" aria-live="off">
      {heroSlides.map((slide, index) => <div key={slide.src.src} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${heroSlides.length}`} aria-hidden={active !== index} data-active={active === index} className={`absolute inset-0 transition-opacity duration-1000 motion-reduce:transition-none ${active === index ? "opacity-100" : "opacity-0"}`}>
        <Image src={slide.src} alt={slide.alt} fill priority={index === 0} loading={index === 0 ? undefined : "eager"} quality={85} sizes="100vw" className="object-cover [object-position:var(--photo-position)] max-sm:[object-position:var(--mobile-photo-position)]" style={{ "--photo-position": slide.position, "--mobile-photo-position": slide.mobilePosition } as CSSProperties} onLoad={() => setLoaded(current => current.includes(index) ? current : [...current, index])} />
      </div>)}
    </div>
    ...
    <div className="relative z-10 mx-auto w-full max-w-5xl">
      <p className="font-imperial text-6xl sm:text-7xl" aria-label="Anjo and Jasmin initials">A &amp; J</p>
      <p className="mt-6 text-xs uppercase tracking-[.4em]">The wedding of</p>
      <h1 className="my-8 flex flex-col items-center justify-center gap-2 font-imperial text-[clamp(5rem,10vw,8rem)] leading-[1.05] md:flex-row md:gap-9"><span>Anjo</span><span className="text-[.5em] text-[#f3c3cf]">&amp;</span><span>Jasmin</span></h1>
      <p className="text-base uppercase tracking-[.12em] sm:text-2xl">{wedding.date}</p>
      <p className="mt-4 text-sm sm:text-base">{wedding.time} · Malabon, Philippines</p>
      <a href="#rsvp" className="mt-8 inline-flex min-h-11 items-center rounded-md bg-[#f3c3cf] px-9 py-3 text-sm tracking-wide text-[#624451] transition hover:bg-[#f8edf0]">RSVP</a>
    </div>
    ...
    <a href="#countdown" aria-label="Explore the invitation" className="absolute bottom-6 left-1/2 z-10 flex h-10 w-6 -translate-x-1/2 justify-center rounded-full border border-white/70 pt-2"><span className="h-2 w-1 rounded-full bg-white" /></a>
  </section>;
}
