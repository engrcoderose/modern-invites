...
export default function CinematicBreak() {
  ...
  const shade = useTransform(progress, [0, 0.35, 0.65, 1], [0.55, 0.42, 0.46, 0.65]);
  ...
  return (
    <section id="cinematic" ref={sectionRef} aria-label="A favorite moment" className="relative h-[190vh] bg-[#f4d1b6]">
      <div className="sticky top-0 h-[100svh] overflow-hidden p-3 sm:p-5 lg:p-8">
        ...
        >
          ...
          <motion.div style={{ opacity: reduceMotion ? 0.46 : shade }} className="absolute inset-0 bg-[#583e4a]" />
          <div className="hero-grain absolute inset-0 opacity-20" />

          <div className="pointer-events-none absolute inset-0" aria-hidden="true">
            <Image src={PinkFlower} alt="" sizes="(max-width: 640px) 112px, 200px" className="absolute -bottom-2 -left-6 h-auto w-28 -rotate-12 opacity-90 sm:-left-4 sm:w-[200px]" />
            <Image src={YellowFlower} alt="" sizes="(max-width: 640px) 88px, 160px" className="absolute -bottom-2 -right-3 h-auto w-[88px] rotate-12 opacity-90 sm:right-2 sm:w-40" />
          </div>
          ...
          >
            <p className="text-[0.58rem] uppercase tracking-[0.38em] text-[#ffe1ee]">A moment we will keep forever</p>
            <h2 className="mt-7 max-w-5xl font-instrumentSerif text-[clamp(3.8rem,9vw,9rem)] leading-[0.82] tracking-[-0.055em]">
              In every lifetime,<br /><span className="font-meaCulpa text-[0.6em] font-normal text-[#ffe1ee]">I would find you.</span>
            </h2>
            <motion.span style={reduceMotion ? undefined : { scaleX: lineScale }} className="mt-9 h-px w-24 origin-center bg-[#ffe1ee]/75" />
          </motion.div>
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between text-[0.52rem] uppercase tracking-[0.28em] text-[#ffe1ee]/85 sm:bottom-8 sm:left-8 sm:right-8">
            <span>Anjo &amp; Jasmin</span><span>Malabon · 2026</span>
          </div>
        </motion.div>
      </div>
    </section>
