...
export default function OpeningScreen({ bride, groom, dateDisplay }: OpeningScreenProps) {
  ...
  return (
    <div
      ...
      className={`opening-screen fixed inset-0 z-[100] h-dvh w-full overflow-hidden bg-[#20050b] text-[#fff8ee]${isClosing ? " opening-screen--closing" : ""}`}
    >
      <Image src={RingFocus} alt="" fill priority sizes="100vw" className="object-cover" />
      <video
      ...
      <div className="relative flex h-full flex-col items-center justify-between px-6 py-10 text-center sm:py-12">
        ...
        <div>
          <p className="opening-reveal font-instrumentSerif text-[clamp(4rem,11vw,9rem)] leading-[0.78] tracking-[-0.055em] drop-shadow-2xl [animation-delay:180ms]">
            {bride}
            <span className="my-2 block font-meaCulpa text-[0.48em] font-normal leading-none text-[#e2c689]">and</span>
            {groom}
          </p>
          <p className="opening-reveal mt-8 text-[0.62rem] uppercase tracking-[0.32em] text-white/75 [animation-delay:360ms]">
            {dateDisplay} · Antipolo, Philippines
          </p>
        </div>

        <div className="opening-reveal [animation-delay:540ms]">
          <button
            type="button"
