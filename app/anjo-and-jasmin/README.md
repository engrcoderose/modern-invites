...
- RSVP uses a unified photo-and-form card with the Nylgen and Kersee search/household/attendance sequence and Joshua and Bea-style sample data. Search Clara del Rosario or Enzo Aguilar for two guests, Lucia Navarro for one guest, or Tomas Alvarado for a family of three. Only full name is required for lookup. Contact and dietary fields are optional. Completion happens only after attendance is answered for every listed guest; repeats show the completed household until Restart demo or refresh. The data and logic are owned locally, with no live API calls or persistent storage.
...
RSVP demo: sample households live in `lib/demo-households.ts`, with name matching, attendance validation, and totals in `lib/demo-rsvp.ts`. Do not put real guest records into these client-bundled files. A future live integration must use the event-scoped server RSVP APIs and its own private guest list. This demo does not require an active event in the backend.
