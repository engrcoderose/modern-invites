...
export default function OutfitGallery({
  ...
  const bgColor = theme === "red" ? "bg-[#fff6d2]" : "bg-[#fff6d2]";
  const borderColor =
    theme === "red" ? "border-yellow-600" : "border-yellow-500";
    ...
    <section className={`relative py-20 ${bgColor} overflow-hidden`}>
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          <div
            className={`grid grid-cols-1 ${photos === 2 ? "md:grid-cols-2" : photos === 3 ? "md:grid-cols-3" : "md:grid-cols-4"} gap-6 md:gap-8`}
          >
            {Array.from({ length: photos }).map((_, index) => (
              ...
              >
                <div
                  className={`relative w-full aspect-[3/4] bg-white p-2 shadow-xl border-4 ${borderColor} ${theme === "yellow" ? "rounded-t-full" : ""}`}
                >
                  <div
                    ...
                    {index < outfitSources.length ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-gray-400 text-sm">
                          {theme === "red" ? "Red Outfit" : "Yellow Outfit"}{" "}
                          {index + 1}
                        </span>
                      </div>
                    )}
