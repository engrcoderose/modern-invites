...
export default function GallerySection({ images }: { images: GalleryImage[] }) {
  ...
  const columns = Array.from({ length: Math.min(3, images.length) }, (_, column) => {
    ...
    // Each sequence is taller than the masked viewport, so its repeat is seamless.
  ...
  return (
    <section id="gallery" className="flow-gallery bg-[#e7edf0] text-[#33473d] px-5 py-24 sm:px-8 sm:py-32 lg:px-12 lg:py-44" aria-labelledby="gallery-title">
      <div className="mx-auto max-w-7xl">
        ...
        <div ref={wall} className="flow-gallery-wall" data-paused={paused || !inView || Boolean(selected)} aria-label="Continuously scrolling wedding photographs">
          {columns.map((column, columnIndex) => (
            <div className="flow-gallery-column min-w-0" key={columnIndex}>
              <div className="flow-gallery-track">
                {[0, 1].map(copy => (
                  <div className="flow-gallery-sequence flex flex-col" key={copy} aria-hidden={copy === 1 ? true : undefined}>
                    {column.map((photo, index) => (
                      <button className="flow-gallery-photo relative block aspect-square w-full min-h-0 shrink-0 cursor-zoom-in overflow-hidden rounded-md border-0 bg-[#d1dad2] p-0" key={`${copy}-${index}`} tabIndex={copy === 1 ? -1 : 0} aria-label={`View photograph: ${photo.alt}`} onClick={event => { opener.current = event.currentTarget; setSelected(photo); }}>
                        <Image src={photo.src} alt={copy === 1 ? "" : photo.alt} fill sizes="(max-width: 640px) 33vw, (max-width: 1280px) 30vw, 410px" style={{ objectPosition: photo.position }} className="object-cover" />
                      </button>
                    ))}
                  </div>
                ))}
      ...
      <dialog ref={dialog} className="flow-gallery-dialog w-[min(1100px,94vw)] max-h-[94svh] border-0 bg-[#fbf8f1] px-[18px] pb-[18px] pt-[45px] text-[#33473d]" onCancel={event => { event.preventDefault(); close(); }} onClick={event => { if (event.target === event.currentTarget) close(); }} aria-label="Wedding photograph preview">
