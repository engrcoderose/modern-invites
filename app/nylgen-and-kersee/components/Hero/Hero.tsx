...
export function Hero({ data }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        aria-hidden="true"
        className="pointer-events-none absolute -left-5 top-[18%] hidden text-wedding-sage/35 lg:block"
        initial={reduceMotion ? false : { opacity: 0, rotate: -18, x: -24 }}
        animate={{ opacity: 1, rotate: -8, x: 0 }}
        transition={{ duration: 1.5, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
      >
        <Sprout className="size-36 stroke-[0.7]" />
      </motion.div>
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-wedding-wide items-center gap-12 px-wedding-gutter pb-12 pt-20 sm:pb-16 sm:pt-24 lg:grid-cols-[0.88fr_1.12fr] lg:gap-16 lg:py-16 xl:gap-24">
        ...
        >
          ...
          >
            ...
            <Leaf className="size-3.5 stroke-[1.25]" aria-hidden="true" />
          ...
          <motion.h1
            ...
            className="font-wedding-display text-[clamp(4.5rem,13vw,9.4rem)] font-normal leading-[0.68] tracking-[-0.065em] text-wedding-sage-deep"
          >
            <span className="block">{couple.groom.firstName}</span>
            <span className="my-2 block translate-x-[0.06em] font-wedding-script text-[0.52em] font-normal leading-none text-wedding-gold lg:my-4 lg:translate-x-[1.1em]">
              &amp;
            </span>
            <span className="block lg:translate-x-[0.28em]">
          ...
          <motion.p
            variants={reveal}
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="mt-4 font-wedding-body text-[0.7rem] tracking-[0.08em] text-wedding-sage sm:text-xs"
          >
            {event.ceremony.name}
          </motion.p>
        </motion.div>
