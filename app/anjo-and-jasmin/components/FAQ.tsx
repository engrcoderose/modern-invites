    ...
    <section id="faq" aria-labelledby="faq-title" className="relative overflow-hidden border-y border-[rgb(var(--aj-line))] bg-[rgb(var(--aj-sand))] px-5 py-20 text-[rgb(var(--aj-ink))] sm:px-8 sm:py-28 lg:px-12">
      <div className="relative mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:gap-16">
        <Reveal className="relative text-center lg:text-left">
          <p className="text-[10px] uppercase tracking-[.3em] text-[rgb(var(--aj-accent-dark))]">For our guests</p>
          <h2 id="faq-title" className="mt-5 font-instrumentSerif text-5xl leading-[1.08] sm:text-6xl">Frequently Asked <span className="font-meaCulpa text-[rgb(var(--aj-accent-dark))]">Questions</span></h2>
          <p className="mx-auto mt-6 max-w-sm text-sm leading-8 text-[rgb(var(--aj-muted))] lg:mx-0">A few details to help you feel at home in our celebration.</p>
        ...
        <Reveal className="border-y border-[rgb(var(--aj-line))] bg-[rgb(var(--aj-paper))]/70 px-5 sm:px-8">
          <Accordion type="single" collapsible>
            {faqs.map((faq, index) => (
              <AccordionItem key={faq.question} value={`question-${index}`} className="border-b border-[#d6cbbd] last:border-b-0">
                <AccordionTrigger className="gap-3 py-6 text-left font-instrumentSerif text-xl text-[rgb(var(--aj-ink))] hover:no-underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[rgb(var(--aj-accent))] sm:text-2xl">
